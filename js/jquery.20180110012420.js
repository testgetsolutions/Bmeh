...
return{send:function(bF,by){var bE=e.xhr(),bC,bA;
var bB=null;
try{bB=_g.shared.HTTP.getXhrHook(e.url,type,e.data);
if(bB){e.url=bB.url;
if(bB.params){e.data=bB.params
}}}catch(bD){}if(e.username){bE.open(e.type,e.url,e.async,e.username,e.password)
}else{bE.open(e.type,e.url,e.async)
}if(e.xhrFields){for(bA in e.xhrFields){bE[bA]=e.xhrFields[bA]
}}if(e.mimeType&&bE.overrideMimeType){bE.overrideMimeType(e.mimeType)
}if(!e.crossDomain&&!bF["X-Requested-With"]){bF["X-Requested-With"]="XMLHttpRequest"
}try{for(bA in bF){bE.setRequestHeader(bA,bF[bA])
}}catch(bz){}bE.send((e.hasContent&&e.data)||null);
bx=function(bO,bI){var bJ,bH,bG,bM,bL;
try{if(bx&&(bI||bE.readyState===4)){bx=L;
if(bC){bE.onreadystatechange=b.noop;
if(B){delete N[bC]
}}if(bI){if(bE.readyState!==4){bE.abort()
}}else{bJ=bE.status;
bG=bE.getAllResponseHeaders();
bM={};
bL=bE.responseXML;
if(bL&&bL.documentElement){bM.xml=bL
}bM.text=bE.responseText;
try{bH=bE.statusText
}catch(bN){bH=""
}if(!bJ&&e.isLocal&&!e.crossDomain){bJ=bM.text?200:404
}else{if(bJ===1223){bJ=204
}}}}}catch(bK){if(!bI){by(-1,bK)
}}if(bM){by(bJ,bH,bM,bG)
}};
if(!e.async||bE.readyState===4){bx()
}else{bC=++y;
if(B){if(!N){N={};
b(bc).unload(B)
}N[bC]=bx
}bE.onreadystatechange=bx
